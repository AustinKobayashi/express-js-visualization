...
def build_regex(route_method, route):
    ...
    if route_method == "get":
        regex['app_get'] = re.compile(
            r'' + re.escape(app_var) + r'\.get\((?:\'|\")' + re.escape(route) + '(?:\'|\"),(.?)')

    if route_method == "put":
        regex['app_put'] = re.compile(
            r'' + re.escape(app_var) + r'\.put\((?:\'|\")' + re.escape(route) + '(?:\'|\"),(.?)')

    if route_method == "post":
        regex['app_post'] = re.compile(
            r'' + re.escape(app_var) + r'\.post\((?:\'|\")' + re.escape(route) + '(?:\'|\"),(.?)')

    if route_method == "delete":
        regex['app_delete'] = re.compile(
            r'' + re.escape(app_var) + r'\.delete\((?:\'|\")' + re.escape(route) + '(?:\'|\"),(.?)')

    if route_method == "patch":
        regex['app_patch'] = re.compile(
            r'' + re.escape(app_var) + r'\.patch\((?:\'|\")' + re.escape(route) + '(?:\'|\"),(.?)')
...
def remove_garbage_from_path(router):
    return router[router.rindex(os.path.sep) + 1:]
